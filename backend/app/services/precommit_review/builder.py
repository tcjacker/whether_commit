...
class PrecommitReviewBuilder:
    def __init__(self, workspace_path: str) -> None:
        ...
        self._state_store: ReviewStateStore | None = None

    @property
    def state_store(self) -> ReviewStateStore:
        if self._state_store is None:
            self._state_store = ReviewStateStore(self.workspace_path)
    ...
    def current(self) -> dict[str, Any]:
        ...
        if latest is None:
            raise FileNotFoundError("PRECOMMIT_REVIEW_NOT_READY")
    ...
    def _with_verification_signals(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        ...
        for run in VerificationRunner(self.workspace_path, ensure=False).runs_for_snapshot(snapshot["snapshot_id"]):
