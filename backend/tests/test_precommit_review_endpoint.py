...
def test_precommit_review_current_does_not_rebuild_when_not_ready(tmp_path):
    repo = make_repo(tmp_path)
    (repo / "backend" / "schema.py").write_text("value = 2\n", encoding="utf-8")
    run_git(repo, "add", "backend/schema.py")

    response = client.get(f"/api/precommit-review/snapshots/current?workspace_path={repo}")

    assert response.status_code == 404
    assert response.json()["detail"] == "PRECOMMIT_REVIEW_NOT_READY"
    assert not (repo / ".precommit-review").exists()
...
def test_precommit_verification_run_endpoint_records_failed_command(tmp_path):
    ...
    response = client.post(
        "/api/precommit-review/verification/run",
        json={
    ...
    poll = client.get(f"/api/precommit-review/verification/runs/{run_id}?workspace_path={repo}")
