...
export function fetchCurrentSnapshot(workspacePath: string): Promise<PrecommitSnapshot> {
  return get<PrecommitSnapshot>(`/api/precommit-review/snapshots/current?${params(workspacePath)}`)
}
...
export function runVerificationCommand(
  ...
  return post<VerificationRun>('/api/precommit-review/verification/run', { workspace_path: workspacePath, snapshot_id: snapshotId, command })
}

export function fetchVerificationRun(workspacePath: string, runId: string): Promise<VerificationRun> {
  return get<VerificationRun>(`/api/precommit-review/verification/runs/${runId}?${params(workspacePath)}`)
}
